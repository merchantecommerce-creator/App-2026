...
export const MeasurementEditor: React.FC<MeasurementEditorProps> = ({ image, onSave, onClose }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // REDUCIDO: de 3 * scale a 1.5 * scale para líneas más delgadas
            ...
            {imageUrl ? (
              ...
              >
                  ...
                  <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 10 }}>
                      <defs>
                          <marker id="arrowhead-end" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto">
                              <polygon points="0 0, 8 4, 0 8" fill="#000000" />
                          </marker>
                          <marker id="arrowhead-start" markerWidth="8" markerHeight="8" refX="1" refY="4" orient="auto">
                      ...
                      {(Object.keys(measurements) as MeasurementType[]).map(key => {
                          ...
                          return (
                              <React.Fragment key={key}>
                                  ...
                                  {/* REDUCIDO: de 3 a 1.5 para líneas más delgadas */}
                                  <line 
                                    x1={`${m.start.x}%`} 
                                    y1={`${m.start.y}%`} 
                                    x2={`${m.end.x}%`} 
